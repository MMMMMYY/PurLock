...
class RNNModel(nn.Module):
    ...
    def forward(self, x):
        out, _ = self.rnn(x)
        return self.fc(out[:, -1])  # 取最后一个时间步的输出作为预测
...
def main(dataset_choice="stock"):
    ...
    # model = RNNModel(input_dim=1, hidden_dim=64, num_layers=2).to(device)
    ...
    for epoch in range(30):
        ...
        test_mse, test_nmse = evaluate_model(model, test_loader, scaler, device)
        results.append({
            "Epoch": epoch,
        ...
        print(f"Epoch {epoch + 1}: Train Loss = {train_loss:.6f} | Test MSE = {test_mse:.6f} | Test NMSE = {test_nmse:.6f}")
